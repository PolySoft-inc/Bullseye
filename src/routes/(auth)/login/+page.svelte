<script>
	...
	const formSchema = z.object({
		...
		password: z.string().min(8, { message: "Must be 8 or more characters long" }).max(50, { message: "Must be 50 characters or less" }),
	});
...
<form method="POST" use:enhance onsubmit={handleSubmit} class="pb-safe pt-safe flex h-dvh flex-col px-8">
	...
	<Form.Field {form} name="password">
		<Form.Control>
			{#snippet children({ props })}
				<Input class="p-7" placeholder="password" {...props} type="password" bind:value={$formData.password} />
			{/snippet}
		</Form.Control>
		<Form.FieldErrors />
	</Form.Field>
